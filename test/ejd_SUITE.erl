...
init_per_suite(Config) ->
	...
	dbg:tracer(),
	dbg:p(all, call),
	dbg:tpl(cowboy_req, body, x),
	dbg:tpl(cowboy_req, stream_body, x),
	dbg:tpl(cowboy_req, stream_body_recv, x),
	dbg:tpl(gen_tcp, recv, x),
	dbg:tpl(http_handler_loop_echo, info, x),
	%
...
init_dispatch() ->
	cowboy_router:compile([
		{"localhost", [
			{"/loop_echo", http_handler_loop_echo, []}
		]}
	]).
...
post_request(Config) ->
	Client = ?config(client, Config),
	Body = <<"<body content='text/xml; charset=utf-8' hold='1' xmlns='http://jabber.org/protocol/httpbind' to='localhost' wait='300' rid='681972' secure='false' newkey='3c7b6c589aa1e29309e4080543e70aead3ed98d0' xml:lang='en' ver='1.6' xmlns:xmpp='urn:xmpp:xbosh' xmpp:version='1.0'/>">>,
	[begin
		{ok, Client2} = cowboy_client:request(<<"POST">>,
			build_url("/loop_echo", Config),
			[],
			Body, Client),
		{ok, 200, W, Client3} = cowboy_client:response(Client2),
		{ok, Body, _} = cowboy_client:response_body(Client3)
	end].
